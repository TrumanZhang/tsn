...
void TransmissionSelection::handleMessage(cMessage* msg) {
  if (msg->isSelfMessage()) {
    ...
    if (par("verbose")) {
      EV_DETAIL << getFullName() << ": msg arrived at gate: " << gateId << " which is express: "
...
void TransmissionSelection::refreshDisplay() const {
  char buf[80];
  sprintf(buf, "packetRequested: %s", packetRequestedFromUs ? "true" : "false");
...
void TransmissionSelection::handleRequestPacketEvent() {
  ...
  if (packetRequestedFromUs) {
    throw cRuntimeError("TransmissionSelection module only supports one packet request at a +"
        "time.");
  }
