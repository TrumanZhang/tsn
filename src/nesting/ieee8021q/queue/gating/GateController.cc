...
void GateController::initialize(int stage) {
  ...
  if (stage == INITSTAGE_LOCAL) {
    ...
    TransmissionGate* transmissionGateVectorModule = getModuleFromPar<TransmissionGate>(
        par("transmissionGateVectorModule"), this);
    cModule::SubmoduleIterator it = cModule::SubmoduleIterator(transmissionGateVectorModule->getParentModule());
    for (; !it.end(); it++) {
      cModule* subModule = *it;
      if (subModule->isName(transmissionGateVectorModule->getName())) {
        TransmissionGate* transmissionGate = check_and_cast<TransmissionGate*>(subModule);
  ...
  //initialize schedule in second stage when clock is initialized
    ...
    switchString = this->getModuleByPath(par("switchModule"))->getFullName();
    portString = to_string(this->getModuleByPath(par("queuingModule"))->getIndex());

    lastChange = simTime();
    currentSchedule = unique_ptr<Schedule<GateBitvector>>(new Schedule<GateBitvector>());
...
unsigned int GateController::calculateMaxBit(int gateIndex) {
  ...
  unsigned int currentIndex = (scheduleIndex + currentSchedule->size() - 1) % currentSchedule->size();
  ...
  while (bits < Ieee8021q::kEthernet2MaximumTransmissionUnitBitLength) {
    ...
    if (!nextSchedule) {
      bits = bits
          + ((currentSchedule->getLength(currentIndex) * clockRate) - timeSinceLastChange)
      ...
      if (!touchedNextSchedule) {
        ...
        bits = bits
            + ((nextSchedule->getLength(currentIndex) * clockRate) - timeSinceLastChange)
                / (SimTime(1, SIMTIME_S) / transmitRate);
        timeSinceLastChange = SIMTIME_ZERO;
...
void GateController::loadScheduleOrDefault(cXMLElement* xml) {
  ...
  if (xml != nullptr && xml->hasChildren()) {
    for (cXMLElement* host : xml->getChildren()) {
      if (strcmp(host->getTagName(), "cycle") != 0 && host->getAttribute("name") == switchString) {
        for (cXMLElement* port : host->getChildrenByTagName("port")) {
          if (port->getAttribute("id") == portString) {
            schedule = ScheduleBuilder::createGateBitvectorSchedule(port);
...
//use the default xml that has no entry, but a default cycle defined
  ...
  EV_DEBUG << getFullPath() << ": Loading schedule. Cycle is " << schedule->getLength() << ". Entry count is "
      << schedule->size() << ". Time is " << clock->getTime().inUnit(SIMTIME_US) << endl;
