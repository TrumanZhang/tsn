...
void ClockBase::initialize() {
  if (simTime().getScaleExp() > SIMTIME_NS) {
    throw cRuntimeError("The simulation time precision has to be at least nanoseconds.");
  }
...
void ClockBase::addScheduledTickListener(unsigned int idleTicks, IClockListener* listener) {
...
void ClockBase::updateTimeOnScheduledTick() {
  ...
  EV_DEBUG << getFullPath() << ": "
          << scheduledTicks.front()->scheduledTick.ticks << " tick(s) elapsed. "
          << "Local time: " << lastLocalTickTimestamp << "." << endl;
...
void ClockBase::updateTime() {
  ...
  assert(lastScheduledTick.ticks != 0 || lastScheduledTick.timestamp == lastGlobalTickTimestamp);
...
void ClockBase::subscribeTick(IClockListener* listener, unsigned int idleTicks) {
  Enter_Method_Silent();
  if (!tick) {
    updateTime();
  }
...
void ClockBase::unsubscribeTicks(IClockListener* listener) {
  for (auto entry : scheduledTicks) {
    entry->listeners.erase(remove(entry->listeners.begin(), entry->listeners.end(), listener), entry->listeners.end());
  }
}
