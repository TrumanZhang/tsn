...
simsignal_t EtherMACFullDuplexPreemptable::preemptCurrentFrameSignal = registerSignal("preemptCurrentFrameSignal");
simsignal_t EtherMACFullDuplexPreemptable::transmittedExpressFrameSignal = registerSignal(
    "transmittedExpressFrameSignal");
simsignal_t EtherMACFullDuplexPreemptable::transmittedPreemptableFrameSignal = registerSignal(
    "transmittedPreemptableFrameSignal");
simsignal_t EtherMACFullDuplexPreemptable::transmittedPreemptableFramePartSignal = registerSignal(
    "transmittedPreemptableFramePartSignal");
simsignal_t EtherMACFullDuplexPreemptable::transmittedPreemptableNonFinalSignal = registerSignal(
    "transmittedPreemptableNonFinalSignal");
simsignal_t EtherMACFullDuplexPreemptable::transmittedPreemptableFinalSignal = registerSignal(
    "transmittedPreemptableFinalSignal");
simsignal_t EtherMACFullDuplexPreemptable::transmittedPreemptableFullSignal = registerSignal(
    "transmittedPreemptableFullSignal");
simsignal_t EtherMACFullDuplexPreemptable::expressFrameEnqueuedWhileSendingPreemptableSignal = registerSignal(
    "expressFrameEnqueuedWhileSendingPreemptableSignal");
  ...
  if (msg->isSelfMessage()) {
    ...
    if (pFrame == nullptr) {
      ...
      if (receivedPreemptedFrame == nullptr
          || strcmp(receivedPreemptedFrame->getName(), pFrame->getCompleteFrame()->getName()) != 0) {
        delete receivedPreemptedFrame;
      ...
      if (receivedPreemptedFrame->getByteLength() == preemptedBytesReceived + 8) {
        //enough bytes from the preempted frame received -> send it up
        processMsgFromNetwork(PK(check_and_cast<EtherPhyFrame *>(receivedPreemptedFrame)));
...
void EtherMACFullDuplexPreemptable::processFrameFromUpperLayer(EtherFrame *frame) {
...
void EtherMACFullDuplexPreemptable::getNextFrameFromQueue() {
  ...
  if (currentExpressFrame) {
    EV_DETAIL << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:"
        << " Queuing express frame now that preemption is finished." << endl;
    curTxFrame = currentExpressFrame;
    currentExpressFrame = nullptr;
    //Check if a new express frame is available
  } else if (par("enablePreemptingFrames") && checkForAndRequestExpressFrame()) {
    return;
    ...
    EV_DETAIL << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:" << " Getting preempted frame "
        << curTxFrame << " instead of one from the queue." << endl;
...
void EtherMACFullDuplexPreemptable::handleEndTxPeriod() {
  ...
  if (par("enablePreemptingFrames") && transmittingPreemptableFrame) {
    ...
    if (preemptedBytesSent + 4 == currentPreemptableFrame->getByteLength()) {
      ...
      if (beginningOfPreemptableFrame) {
        ...
        emit(transmittedPreemptableFinalSignal, currentPreemptableFrame);
      }
    } else {
      emit(transmittedPreemptableNonFinalSignal, currentPreemptableFrame);
    }
    EV_INFO << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:" << " PreemptedFrame "
        << currentPreemptableFrame << " transmitted: " << bytesSentInThisPart << "B, " << preemptedBytesSent << "/"
        << currentPreemptableFrame->getByteLength() << "B" << endl;
    PreemptedFrame* preemptedBytesSentMessage = new PreemptedFrame(encapsulate(currentPreemptableFrame->dup()));
    preemptedBytesSentMessage->setBytesSent(preemptedBytesSent);
    preemptedBytesSentMessage->setBytesTotal(currentPreemptableFrame->getByteLength());
    ...
    if (dynamic_cast<cDelayChannel *>(transmissionChannel) != nullptr) {
      cDelayChannel* delayChannel = check_and_cast<cDelayChannel *>(transmissionChannel);
    ...
    } else if (dynamic_cast<cDatarateChannel *>(transmissionChannel) != nullptr) {
      cDatarateChannel* dataRateChannel = check_and_cast<cDatarateChannel *>(transmissionChannel);
      ...
      if (dynamic_cast<cIdealChannel *>(transmissionChannel) == nullptr) {
        EV_WARN << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:"
            << " Unsupported channel configured. Zero-time PreemptedFrame messages may be delayed." << endl;
      }
      send(preemptedBytesSentMessage, physOutGate);
    }
    ...
    EV_DETAIL << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:" << " Express Frame " << curTxFrame
...
void EtherMACFullDuplexPreemptable::handleEndIFGPeriod() {
  ...
  EV_DETAIL << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:" << " IFG elapsed" << endl;
...
bool EtherMACFullDuplexPreemptable::checkForAndRequestExpressFrame() {
  ...
  if (!transmittingExpressFrame && !transmissionSelectionModule->isEmpty()
    ...
    EV_INFO << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:"
        << " Currently not transmitting or preemption is possible: requesting express frame." << endl;
...
void EtherMACFullDuplexPreemptable::preemptCurrentFrame() {
  ...
  EV_INFO << getFullPath() << " at t=" << simTime().inUnit(SIMTIME_NS) << "ns:" << " Preempting current frame "
...
int EtherMACFullDuplexPreemptable::calculatePreemptedPayloadBytesSent(simtime_t timeToCheck) {
  ...
  int bytesTransmittedInTotal = (timeElapsed / (SimTime(1, SIMTIME_S) / transmitRate)) / 8;
  bytesTransmittedInTotal -= 12;  //subtract preamble (7B), SFD (1B) and checksum (4B)
  if (bytesTransmittedInTotal < 0) {
    return 0;
  } else if (bytesTransmittedInTotal >= currentPreemptableFrame->getByteLength()) {
    throw cRuntimeError("Supposedly transmitted more bytes than the frame's length.");
  }
...
bool EtherMACFullDuplexPreemptable::isPreemptionNowPossible() {
  ...
  int payloadBytesRemaining = currentPreemptableFrame->getByteLength() - payloadBytesSent - preemptedBytesSent;
  if (payloadBytesRemaining < Ieee8021q::kFramePreemptionMinFinalPayloadSize) {
    //Final fragment size would be too short -> Preemption forbidden
    return false;
  } else if (payloadBytesSent >= Ieee8021q::kFramePreemptionMinNonFinalPayloadSize) {
...
simtime_t EtherMACFullDuplexPreemptable::calculateTransmissionDuration(int bytes) {
  double transmitRate = getTxRate();
  ASSERT(transmitRate > 0);
...
simtime_t EtherMACFullDuplexPreemptable::isPreemptionLaterPossible() {
  ...
  int payloadBytesRemaining = currentPreemptableFrame->getByteLength() - payloadBytesSentByNow - preemptedBytesSent;
  if (payloadBytesSentByNow < Ieee8021q::kFramePreemptionMinNonFinalPayloadSize
      && payloadBytesRemaining
          >= Ieee8021q::kFramePreemptionMinNonFinalPayloadSize + 4 - payloadBytesSentByNow
              + Ieee8021q::kFramePreemptionMinFinalPayloadSize) {
    //Preemption not yet possible, but after a short time -> Need to wait to preempt
    int bytesToWait = (Ieee8021q::kFramePreemptionMinNonFinalPayloadSize - payloadBytesSentByNow);
...
void EtherMACFullDuplexPreemptable::refreshDisplay() const {
  ...
  const char* currentPreemptableFrameName =
      (nullptr == currentPreemptableFrame) ? "nullptr" : currentPreemptableFrame->getName();
  const char* currentExpressFrameName = (nullptr == currentExpressFrame) ? "nullptr" : currentExpressFrame->getName();
  sprintf(buf,
      "onHold: %s\ntransmittingExpressFrame: %s\ntransmittingPreemptableFrame: %s\ncurrentPreemptableFrame: %s\ncurrentExpressFrame: %s",
      onHold ? "true" : "false", transmittingExpressFrame ? "true" : "false",
      transmittingPreemptableFrame ? "true" : "false", currentPreemptableFrameName, currentExpressFrameName);
